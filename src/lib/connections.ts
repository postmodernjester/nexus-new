...
// Used by /connect/[userId] page. Uses a SECURITY DEFINER Postgres function
// so it can create contact cards for both users (bypasses RLS).
export async function connectDirectly(
  currentUserId: string,
  targetUserId: string,
  existingContactId?: string | null,
): Promise<{ success: boolean; error?: string }> {
  const { data, error } = await supabase.rpc('connect_users', {
  ...
  if (error) {
    console.error('Error in connect_users RPC:', error);
