...
export default function ChronicleCanvas() {
  ...
  const timelineItems = useMemo<TimelineItem[]>(() => {
    ...
    entries.forEach(e => {
      items.push({
        id: e.id, cat: e.canvas_col || e.type, title: e.title,
    ...
    contacts.forEach(c => {
      items.push({
        id: `contact-${c.id}`, cat: 'people', title: c.full_name,
        start: c.created_at?.slice(0, 7) || '2020-01', end: null,
  ...
  const placeItems = useMemo<PlaceItem[]>(() => {
    return places.map(p => ({
      id: p.id, title: p.title, start: p.start_date, end: p.end_date,
      color: p.color || '#c0b484', fuzzyStart: p.fuzzy_start, fuzzyEnd: p.fuzzy_end,
      note: p.note || '',
    }))
  }, [places])
  ...
  const axisTicks = useMemo(() => {
    ...
    for (let y = START_YEAR; y <= END_YEAR; y++) {
      ...
      ticks.push({ y, top: toPx({ y, m: 1 }, pxm), age: y - BIRTH_YEAR, isDecade: y % 10 === 0, isFive: y % 5 === 0 })
    }
    return ticks
  }, [pxm])
  ...
  const selectEntry = useCallback((id: string | null) => {
    setSelectedId(id)
    if (id) { setShowDelHint(true); setTimeout(() => setShowDelHint(false), 2000) }
  }, [])
  ...
  useEffect(() => {
    const handler = (ev: KeyboardEvent) => {
      ...
      if (ev.key === 'Escape') {
        setSelectedId(null); setEntryModal({ open: false }); setGeoModal({ open: false })
      }
    }
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      if (d.type === 'move') { newTop = snap(Math.max(0, d.origTop + dy)); newH = snap(d.origH) }
      else if (d.type === 'top') { newTop = snap(Math.max(0, d.origTop + dy)); newH = snap(Math.max(pxm, d.origH - dy)) }
      else { newTop = snap(d.origTop); newH = snap(Math.max(pxm, d.origH + dy)) }
      const ns = pxToYM(newTop, pxm), ne = pxToYM(newTop + newH, pxm)
      const newStart = ymStr(ns), newEnd = ymStr(ne)
      setEntries(prev => prev.map(e => e.id === d.id ? { ...e, start_date: newStart, end_date: newEnd } : e))
      updateEntryDates(d.id, newStart, newEnd).catch(console.error)
      dragRef.current = null; setDragDelta(null); setTooltip(null)
    }
  ...
  const zoomLabel = scale >= 1 ? scale.toFixed(1).replace(/\.0$/, '') + '\u00d7' : (scale * 100).toFixed(0) + '%'
  ...
  useEffect(() => {
    const handleMouseMove = (ev: MouseEvent) => { if (zoomDragRef.current) zoomFromTrack(ev.clientX) }
  ...
  const handleBackgroundClick = useCallback((ev: React.MouseEvent) => {
    if (!(ev.target as HTMLElement).closest('[data-entry],[data-person]')) setSelectedId(null)
  }, [])
  ...
  const handleGridDblClick = useCallback((ev: React.MouseEvent) => {
    ...
    setEntryModal({ open: true, editing: null, defaultCat: COLS[colIdx].id, defaultYM: ymStr(ym) })
  }, [pxm])
  ...
  const handleAxisClick = useCallback((ev: React.MouseEvent) => {
    ...
    if (hit) {
      setGeoModal({ open: true, editing: { id: hit.id, title: hit.title, start: hit.start, end: hit.end || '', fuzzyStart: hit.fuzzyStart, fuzzyEnd: hit.fuzzyEnd, color: hit.color } })
    } else {
      setGeoModal({ open: true, editing: null, defaultYM: ymStr(ym) })
    }
  }, [pxm, placeItems])
  ...
  const handleEntryDelete = useCallback(async (id: string) => {
    try {
      await deleteEntry(id); setEntries(prev => prev.filter(e => e.id !== id))
      setSelectedId(null); setEntryModal({ open: false })
  ...
  const openEditModal = useCallback((item: TimelineItem) => {
    ...
    setEntryModal({
      open: true, editing: {
        id: entry.id, cat: entry.canvas_col || entry.type, title: entry.title,
  ...
  const handleGeoSave = useCallback(async (data: GeoFormData) => {
    try {
      const result = await upsertPlace({
        id: data.id || undefined, title: data.title,
        start_date: data.start, end_date: data.end || null,
        color: data.color, fuzzy_start: data.fuzzyStart, fuzzy_end: data.fuzzyEnd, show_on_resume: false,
      })
      setPlaces(prev => { if (data.id) return prev.map(p => p.id === data.id ? result : p); return [...prev, result] })
  ...
  const handleGeoDelete = useCallback(async (id: string) => {
    try {
      await deletePlace(id); setPlaces(prev => prev.filter(p => p.id !== id)); setGeoModal({ open: false })
    } catch (err) { console.error('Failed to delete place:', err) }
  }, [])
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#f0ead8', color: '#1a1812', fontFamily: "'DM Mono', monospace", userSelect: 'none' }}>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', borderBottom: '2px solid #1a1812', background: '#f6f1e6', zIndex: 60, height: 40 }}>
        <div style={{ padding: '0 14px', borderRight: '1px solid #d8d0c0', height: '100%', display: 'flex', alignItems: 'center', fontFamily: "'Libre Baskerville', serif", fontStyle: 'italic', fontSize: 12, color: '#5a5040' }}>
          chronicle
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '0 14px', borderRight: '1px solid #d8d0c0', height: '100%' }}>
          ...
          <div id="chr-zoom-track" onMouseDown={(e) => { zoomDragRef.current = true; zoomFromTrack(e.clientX); e.preventDefault() }} style={{ position: 'relative', width: 110, height: 3, background: '#d8d0c0', borderRadius: 2, cursor: 'pointer' }}>
            ...
            <div style={{ position: 'absolute', top: '50%', left: `${zoomPct * 100}%`, transform: 'translate(-50%,-50%)', width: 12, height: 12, borderRadius: '50%', background: '#1a1812', cursor: 'grab', boxShadow: '0 1px 4px rgba(0,0,0,.25)' }} />
          </div>
      ...
      <div style={{ flexShrink: 0, display: 'flex', borderBottom: '1.5px solid #1a1812', background: '#f6f1e6', zIndex: 50 }}>
        ...
        <div style={{ display: 'flex' }}>
          {COLS.map(col => (
            <div key={col.id} onDoubleClick={() => setEntryModal({ open: true, editing: null, defaultCat: col.id })} style={{ flexShrink: 0, width: COL_W, borderRight: '1px solid #d8d0c0', padding: '7px 10px', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
      ...
      <div ref={scrollRef} onClick={handleBackgroundClick} style={{ flex: 1, overflowY: 'scroll', overflowX: 'auto', position: 'relative' }}>
        <div style={{ display: 'flex', position: 'relative', minWidth: AXIS_W + COLS.length * COL_W }}>

          <div id="chr-axis" onClick={handleAxisClick} style={{ width: AXIS_W, flexShrink: 0, borderRight: '2px solid #1a1812', position: 'sticky', left: 0, zIndex: 30, background: '#f6f1e6', height: totalH, cursor: 'crosshair' }}>
            {axisTicks.map(t => (
          ...
          <div id="chr-grid" onDoubleClick={handleGridDblClick} style={{ flex: 1, position: 'relative', height: totalH, width: COLS.length * COL_W }}>
            {yearRules.map(r => (
              <div key={`yr-${r.y}`} style={{ position: 'absolute', left: 0, right: 0, top: r.top, height: r.isDecade ? 2 : 1, background: `rgba(0,0,0,${r.isDecade ? 0.18 : 0.1})`, pointerEvents: 'none', zIndex: 2 }} />
            ))}
            {monthRules.map((r, i) => (
              <div key={`mo-${i}`} style={{ position: 'absolute', left: 0, right: 0, top: r.top, height: 1, background: 'rgba(0,0,0,.04)', pointerEvents: 'none', zIndex: 2 }} />
            ))}
            {COLS.map((_, i) => i > 0 && (
              <div key={`col-${i}`} style={{ position: 'absolute', top: 0, bottom: 0, left: i * COL_W, width: 1, background: 'rgba(0,0,0,.07)', pointerEvents: 'none', zIndex: 2 }} />
            ))}
            <div style={{ position: 'absolute', left: 0, right: 0, top: todayTop, height: 2, background: '#c84030', zIndex: 18, pointerEvents: 'none' }}>
            ...
            {placeItems.map(p => {
              const s = parseYM(p.start), en = p.end ? parseYM(p.end) : { y: END_YEAR, m: 1 }
              if (!s || !en) return null
              const top = toPx(s, pxm), h = toPx(en, pxm) - top
              return (
                <div key={p.id}>
                  <div onDoubleClick={() => setGeoModal({ open: true, editing: { id: p.id, title: p.title, start: p.start, end: p.end || '', fuzzyStart: p.fuzzyStart, fuzzyEnd: p.fuzzyEnd, color: p.color } })} style={{ position: 'absolute', left: 0, right: 0, top, height: h, background: hex2rgba(p.color, 0.2), borderTop: !p.fuzzyStart ? `1.5px solid ${hex2rgba(p.color, 0.35)}` : undefined, pointerEvents: 'auto', zIndex: 1, cursor: 'pointer' }}>
                    {p.fuzzyStart && <div style={{ position: 'absolute', left: 0, right: 0, top: 0, height: 20, background: `linear-gradient(to bottom, transparent, ${hex2rgba(p.color, 0.2)})`, pointerEvents: 'none' }} />}
                    {p.fuzzyEnd && <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 20, background: `linear-gradient(to top, transparent, ${hex2rgba(p.color, 0.2)})`, pointerEvents: 'none' }} />}
                  </div>
                  {h > pxm * 2 && <div style={{ position: 'absolute', left: 6, top: top + 6, fontSize: Math.max(6, Math.min(8, pxm * 0.3)), letterSpacing: '.12em', textTransform: 'uppercase', fontStyle: 'italic', color: p.color, pointerEvents: 'none', zIndex: 3, opacity: 0.4 }}>{p.title}</div>}
                </div>
              )
            })}
            ...
            {peopleItems.map(item => {
              const s = parseYM(item.start)
              if (!s) return null
              const top = toPx(s, pxm), dotH = Math.max(14, pxm * 0.9), left = colLeft('people')
              const tailH = toPx({ y: END_YEAR, m: 1 }, pxm) - top - dotH / 2
              return (
                <div key={item.id} data-person="true" onClick={(e) => { e.stopPropagation(); selectEntry(item.id) }} onDoubleClick={(e) => { e.stopPropagation(); openEditModal(item) }} onMouseEnter={(e) => showTooltip(e, item)} onMouseMove={moveTooltip} onMouseLeave={hideTooltip} style={{ position: 'absolute', top, left, width: COL_W, height: dotH, zIndex: 12, cursor: 'default', display: 'flex', alignItems: 'center' }}>
                  <div style={{ width: 9, height: 9, borderRadius: '50%', flexShrink: 0, marginLeft: 6, ...(item.fuzzyStart ? { background: 'transparent', border: `2px dashed ${item.color}`, boxSizing: 'border-box' as const } : { background: item.color }), ...(selectedId === item.id ? { outline: '2px solid #1a1812', outlineOffset: 2 } : {}) }} />
                  <div style={{ fontSize: 8, marginLeft: 7, whiteSpace: 'nowrap', letterSpacing: '.02em', color: item.color }}>{item.title}</div>
                  {!item.end && tailH > 0 && <div style={{ position: 'absolute', left: 10, top: dotH / 2, width: 2, height: tailH, background: item.color, opacity: 0.18, pointerEvents: 'none' }} />}
                </div>
              )
            })}

            {regularItems.map(item => {
              const s = parseYM(item.start), en = item.end ? parseYM(item.end) : null
              if (!s) return null
              const dragged = getDraggedPosition(item)
              let top: number, h: number
              if (dragged) { top = dragged.top; h = dragged.h }
              else { top = toPx(s, pxm); h = en ? Math.max(Math.round(pxm), toPx(en, pxm) - top) : Math.round(pxm * 2) }
              const left = colLeft(item.cat) + 3, w = COL_W - 7
              const showDate = h > Math.round(pxm * 1.2), isSelected = selectedId === item.id
              return (
                <div key={item.id} data-entry="true" style={{ position: 'absolute', top, left, width: w, height: h, borderRadius: 3, overflow: 'visible', zIndex: 10, cursor: 'default' }}>
                  ...
                  {item.source === 'chronicle' && (
                    <>
                      <div onMouseDown={(e) => startDrag(e, item, 'top')} style={{ position: 'absolute', left: 0, right: 0, height: 8, top: -4, zIndex: 15, cursor: 'ns-resize', opacity: isSelected ? 1 : 0, transition: 'opacity .15s' }}>
                      ...
                      <div onMouseDown={(e) => startDrag(e, item, 'bot')} style={{ position: 'absolute', left: 0, right: 0, height: 8, bottom: -4, zIndex: 15, cursor: 'ns-resize', opacity: isSelected ? 1 : 0, transition: 'opacity .15s' }}>
                        <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 24, height: 3, borderRadius: 2, background: 'rgba(0,0,0,.28)' }} />
                      </div>
                    </>
                  )}
      ...
      {tooltip && <div style={{ position: 'fixed', left: tooltip.x, top: tooltip.y, background: '#1a1812', color: '#f6f1e6', padding: '5px 10px', borderRadius: 3, fontSize: 9, letterSpacing: '.04em', pointerEvents: 'none', zIndex: 400, maxWidth: 280, lineHeight: 1.6 }}>{tooltip.text}</div>}

      <div style={{ position: 'fixed', bottom: 16, left: '50%', transform: 'translateX(-50%)', background: '#1a1812', color: '#f6f1e6', padding: '5px 14px', borderRadius: 20, fontSize: 8, letterSpacing: '.1em', opacity: showDelHint ? 1 : 0, pointerEvents: 'none', transition: 'opacity .2s', zIndex: 300 }}>DEL to delete selected</div>

      <ChronicleModal open={entryModal.open} editingEntry={entryModal.editing} defaultCat={entryModal.defaultCat} defaultYM={entryModal.defaultYM} onSave={handleEntrySave} onDelete={handleEntryDelete} onClose={() => setEntryModal({ open: false })} />
      <ChronicleGeoModal open={geoModal.open} editingPlace={geoModal.editing} defaultYM={geoModal.defaultYM} onSave={handleGeoSave} onDelete={handleGeoDelete} onClose={() => setGeoModal({ open: false })} />
    </div>
  )
}
